...
<div class="row">
    <div class="col-md-12"><h1>Changer l'ordre</h1></div>
</div>
<div class="row">
    <div class="col-md-12">
        <div class="row">
            <div class="col-md-6">
                <p><a href="{{ url('admin/campagne/'.$campagne->id) }}" class="btn btn-default"><i class="fa fa-chevron-left"></i> &nbsp;Composer la campagne</a></p>
            </div>
            <div class="col-md-6">
                @if(!$clipboards->isEmpty())
                    <form action="{{ url('admin/clipboard/paste') }}" class="pull-right" method="POST">{!! csrf_field() !!}
                    ...
                    <div id="sortable_list">
                        @foreach($contents as $bloc)
                            <div class="bloc_rang bloc_move" id="bloc_rang_{{ $bloc->id }}" data-rel="{{ $bloc->id }}">
                                <h4>
                                    {{ $bloc->content_title }}
                                    <form action="{{ url('admin/clipboard/copy') }}" class="pull-right" method="POST">{!! csrf_field() !!}
